 ...
 */
export async function encryptWithKey<R>(key: CryptoKey, dataObj: R): Promise<EncryptionResult> {
 ...
 */
export async function decrypt(password: string, text: string, key?: CryptoKey): Promise<unknown> {
 ...
 */
export async function decryptWithDetail(password: string, text: string): Promise<DetailedDecryptResult> {
  const payload = JSON.parse(text);
  const {salt} = payload;
  const key = await keyFromPassword(password, salt, true);
  const exportedKeyString = await exportKey(key);
 ...
 * @returns The decrypted data.
 */
export async function decryptWithKey<R>(key: CryptoKey, payload: EncryptionResult): Promise<R> {
...
export async function importKey(keyString: string): Promise<CryptoKey> {
  const key = await crypto.subtle.importKey(EXPORT_FORMAT, JSON.parse(keyString), DERIVED_KEY_FORMAT, true, [
    'encrypt',
    'decrypt',
  ]);
 ...
 */
export async function keyFromPassword(password: string, salt: string, exportable = false): Promise<CryptoKey> {
